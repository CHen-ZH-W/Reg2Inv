...
def preprocess_pcd(pcd, norm_size):
    pcd = pcd / norm_size
    _, down_pcd = binary_search_voxel_size(pcd, 4096, 0.01)
...
def norm_pcd(template, num_points):
    ...
    _, down_pcd = binary_search_voxel_size(template, 4096, 0.01)
...
def norm_pcd_train(template, num_points):
    ...
    voxel_size, _ = binary_search_voxel_size(template_norm, num_points, 0.01)

    return template_norm, norm_size, voxel_size

def add_gaussian_noise_to_point_cloud(point_cloud, std_dev=0.001):

    noisy_pc = point_cloud.copy()
    noise = np.random.normal(0, std_dev, size=point_cloud[:, :3].shape)
    noisy_pc[:, :3] += noise
    
    return noisy_pc

class Real3dadPairDataset_train(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(Real3dadPairDataset_train, self).__init__()
...
class Real3dadPairDataset_test(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(Real3dadPairDataset_test, self).__init__()
...
class Real3dadDataset_train(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(Real3dadDataset_train, self).__init__()
