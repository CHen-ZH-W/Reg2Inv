...
class SuperPointMatching(nn.Module):
    ...
    def forward(self, ref_feats, src_feats, ref_masks=None, src_masks=None):
        ...
        ref_sel_indices = torch.div(corr_indices, matching_scores.shape[1], rounding_mode='trunc')
        src_sel_indices = corr_indices % matching_scores.shape[1]
        '''
        src_max_scores, src_max_indices = matching_scores.max(dim=0)
        sorted_scores, sorted_indices = torch.sort(src_max_scores, descending=True)
        topk_indices = sorted_indices[:num_correspondences]
        src_sel_indices = topk_indices
        ref_sel_indices = src_max_indices[topk_indices]
        corr_scores = sorted_scores[:num_correspondences]
        '''
