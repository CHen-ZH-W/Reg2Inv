...
def precompute_data_stack_mode_two(ref_points, src_points, ref_lengths, src_lengths, num_stages, voxel_size, init_radius, neighbor_limits):
    assert num_stages == len(neighbor_limits)

    ref_points_list = []
    ref_lengths_list = []
    ref_neighbors_list = []
    ref_subsampling_list = []
    ref_upsampling_list = []

    src_points_list = []
    src_lengths_list = []
    src_neighbors_list = []
    src_subsampling_list = []
    src_upsampling_list = []

    # grid subsampling
    for i in range(num_stages):
        if i > 0:
            ref_points, ref_lengths = grid_subsample(ref_points, ref_lengths, voxel_size=voxel_size)
            src_points, src_lengths = grid_subsample(src_points, src_lengths, voxel_size=voxel_size)
    ...
    radius = init_radius
    for i in range(num_stages):
        cur_points = ref_points_list[i]
        cur_lengths = ref_lengths_list[i]
        ...
        ref_neighbors_list.append(neighbors)

        if i < num_stages - 1:
            sub_points = ref_points_list[i + 1]
            sub_lengths = ref_lengths_list[i + 1]
            ...
            ref_subsampling_list.append(subsampling)
            ...
            ref_upsampling_list.append(upsampling)

        radius *= 2
    
    radius = init_radius
    for i in range(num_stages):
        cur_points = src_points_list[i]
        cur_lengths = src_lengths_list[i]
        ...
        src_neighbors_list.append(neighbors)

        if i < num_stages - 1:
            sub_points = src_points_list[i + 1]
            sub_lengths = src_lengths_list[i + 1]
            ...
            src_subsampling_list.append(subsampling)
            ...
            src_upsampling_list.append(upsampling)

        radius *= 2
    
    return {
        'ref_points': ref_points_list,
        'ref_neighbors': ref_neighbors_list,
        'ref_subsampling': ref_subsampling_list,
        'ref_upsampling': ref_upsampling_list,
        'src_points': src_points_list,
        'src_neighbors': src_neighbors_list,
        'src_subsampling': src_subsampling_list,
        'src_upsampling': src_upsampling_list,
    }
...
def registration_collate_fn_stack_mode(
    data_dicts, num_stages, neighbor_limits, precompute_data=True
):
    ...
    if precompute_data:
        ...
        lengths = torch.LongTensor([points.shape[0] for points in points_list])
        ref_points = points_list[0]
        src_points = points_list[1]
        ref_lengths = lengths[0].unsqueeze(0)
        src_lengths = lengths[1].unsqueeze(0)
        voxel_size = collated_dict['voxel_size'][0]
    ...
    if precompute_data:
        input_dict = precompute_data_stack_mode_two(ref_points,src_points,ref_lengths,src_lengths,num_stages,voxel_size,search_radius,neighbor_limits)
        collated_dict.update(input_dict)
    collated_dict['batch_size'] = batch_size
...
def calibrate_neighbors_stack_mode(
    dataset, collate_fn, num_stages, keep_ratio=0.8, sample_threshold=2000
):
    # Compute higher bound of neighbors number in a neighborhood
    voxel_size = dataset[0]['voxel_size']
    search_radius = voxel_size * 2.5
    ...
    for i in range(len(dataset)):
        data_dict = collate_fn(
            [dataset[i]], num_stages, max_neighbor_limits, precompute_data=True
        )

        # update histogram
        counts = [np.sum(neighbors.numpy() < neighbors.shape[0], axis=1) for neighbors in data_dict['ref_neighbors']]
    ...
    return neighbor_limits, voxel_size
