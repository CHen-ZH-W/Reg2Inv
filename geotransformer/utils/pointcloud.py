...
def random_sample_rotation(rotation_magnitude: float) -> np.ndarray:
    angle_radians = np.radians(rotation_magnitude)
    axis = random.choice(['x', 'y', 'z'])
    rotation = Rotation.from_euler(axis, angle_radians).as_matrix()
    transform = np.eye(4)
    transform[:3, :3] = rotation
    return transform
